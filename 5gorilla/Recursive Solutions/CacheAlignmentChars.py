...
def align(initialTop, initialBot):
	...
	def optAlign(topDrop, top, botDrop, bot):
		if (topDrop == topLen and botDrop == botLen):
			...
			(score, td, bd, thisTop, thisBot) = cachedAlignment[topDrop][botDrop]
			(restScore, restTop, restBot) = optAlign(topDrop + td, top[td:], botDrop + bd, bot[bd:])
			return (score, thisTop + restTop, thisBot + restBot)
		else:
			result = (0, "", "")
			td = -1
			bd = -1
